...
`SKILL.md`. Focus is the text after `$my-plan:audit` in the user's request.
...
0. **Always** read `<pluginRoot>/internal/stages/project.md`'s "Where
...
1. **No Working Profile or Project Setup?** Read the rest of
   `<pluginRoot>/internal/stages/project.md` and run setup first. Do not
   send the user to a separate command.

2. **Understand before judging.** Read
   `<pluginRoot>/internal/stages/discovery-spec.md` and run its Quick
...
3. **Audit.** Read `<pluginRoot>/internal/stages/review.md` and dispatch
