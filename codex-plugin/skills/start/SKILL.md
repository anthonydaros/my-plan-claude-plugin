...
Let `<pluginRoot>` be the absolute directory two levels above this loaded
`SKILL.md`. The goal argument is the text after `$my-plan:start` in the user's
request; no following text means resume.
...
| Phase | Read |
|-------|------|
| Setup, project facts | `<pluginRoot>/internal/stages/project.md` |
| Discovery, questions, spec | `<pluginRoot>/internal/stages/discovery-spec.md` |
| Plan and plan review | `<pluginRoot>/internal/stages/planning.md` |
| Build, validate, integrate | `<pluginRoot>/internal/stages/implementation.md` |
| Code review and remediation | `<pluginRoot>/internal/stages/review.md` |
...
Also read `<pluginRoot>/internal/codex.md` during setup, or immediately before
dispatching the first Worker when setup already exists. It owns model resolution,
capability probes, child-session transport, and failure handling.
...
- Never override the repository's Git identity, add a `Co-Authored-By` trailer, or
  ...
  user's.
- The Worker and model that write never review their own work. Not in any retry
  or escalation path.
