...
`SKILL.md`. The Run argument is the text after `$my-plan:exec` in the user's
...
| Phase | Read |
|-------|------|
| Worktree, tasks, validation, commit, push | `<pluginRoot>/internal/stages/implementation.md` |
| Reviews, QA gate, approval | `<pluginRoot>/internal/stages/review.md` |

Do not read both up front. Also read `<pluginRoot>/internal/codex.md` before
dispatching your first Worker. It owns model resolution, child-session
...
- Never override the repository's Git identity, add a `Co-Authored-By` trailer, or
  ...
  user's.
- The Worker that writes never reviews its own work. Not in any fallback.
