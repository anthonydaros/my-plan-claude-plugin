...
`CLAUDE.md` or `AGENTS.md`, generates platform-native skills, and installs hooks
— and `project.md` says this product never modifies `CLAUDE.md`, `AGENTS.md`,
...
`CRG_TOOLS` is not a convenience. It is the only place the write boundary can be
enforced for a Worker that is not a Claude agent: a Codex or opencode Worker has
no `tools:` frontmatter, so the transport is all that stands between it and
`apply_refactor_tool`. Every name absent from that list is unreachable for every
Worker in every backend.
...
The `tools:` list in each agent file is the enforcement; this table is why it
says what it says.
...
| Both reviewers | Change impact, affected flows, untested hotspots, surprising coupling — the same list in both files, byte for byte |
| Committer | Nothing. It runs Git and scans for secrets; it has no reason to read a codebase |

The two reviewers share one list because a lens that depends on which reviewer
was dispatched is not a lens. `smoke-posix.sh` checks the two lists are equal.

## Never granted to a Worker

| Tool | Why |
|------|-----|
| `apply_refactor_tool` | Edits source. A second write path the Coordinator does not check against the approved write set |
...
These are absent from `CRG_TOOLS` as well as from every `tools:` list. Two
independent places, because the frontmatter does not reach Codex Workers and the
transport does not survive a user editing their own MCP configuration.
