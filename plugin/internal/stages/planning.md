...
- `plan.md`, rendered from
  `${CLAUDE_PLUGIN_ROOT}/internal/templates/documents/plan.md.tpl` into
...
- One file per task, rendered from
  `${CLAUDE_PLUGIN_ROOT}/internal/templates/documents/task.md.tpl` into
...
The planner writes into no repository, in either backend: no worktree exists,
and the primary checkout is not its to touch. In hybrid that boundary is a
process-enforced read-only sandbox; in claude-only it is a write set that names
Run state only.

**Hybrid.** Dispatch the Codex Worker with `--sandbox read-only`, `-C` at the
primary checkout, the prompt rendered from
`${CLAUDE_PLUGIN_ROOT}/internal/prompts/plan-write.tpl`, and `--output-schema`
pointed at `${CLAUDE_PLUGIN_ROOT}/internal/contracts/plan-result.schema.json`.
...
Verify the result before writing anything: `taskCount` equals the number of
task entries, `planContent` ends with the exact line `END OF PLAN`, and no
`{{` placeholder survives anywhere in the returned content. A result failing
any of these is a truncated or failed attempt against the same candidate, never
a plan. Then write `plan.md` and the task files from the returned content, byte
for byte. The Worker never writes; the files are yours.

**Claude-only.** The `my-plan-planner` agent writes the same two destinations
itself — its write set is the artifacts `plan.md` and `tasks/` paths, nothing in
any repository — and you verify the same completeness properties on disk: the
terminator line, no surviving `{{`, and a task file for every row of the plan's
task table.

**Greenfield Mode.** The planner's workspace is the empty target directory;
the specification, discovery record, and research carry all the evidence, and
the Architecture Memory and project skill do not exist yet. Send what exists.
...
- The reference paths the writer needs — helpers to reuse, patterns to match,
  contracts to satisfy, the tests that pin behavior — so execution starts from
  the file, never from a search of the codebase.

Read each task file back asking what a writer would have to guess. Whatever it
is, write it down or make it a blocker. A guess that turns out right still cost
the review that had to verify it; a guess that turns out wrong costs a
remediation round and arrives disguised as working code.
...
`high` via Codex, read-only sandbox, `-C` at the primary checkout, per
...
trusting it. Check that `subjectHash` matches what you sent: the plan hash, in
the pinned form `project.md` defines — `plan.md` and every task file together.
Invalid or unparseable output is a failed attempt, never an implicit approval.
