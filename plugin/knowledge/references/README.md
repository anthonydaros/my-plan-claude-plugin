...
`checklists/review.md` decide what is a finding, the report template one
level up decides the shape of a report, and these files only say what the
defect looks like in a given stack.
...
these files exist so a review can be specific without carrying twenty guides it
...
| Writing a finding someone can act on | `code-review-best-practices.md` | rarely: the report template and the checklist already say this |
...
one still does, held to a stricter evidence bar, and the difference matters most
