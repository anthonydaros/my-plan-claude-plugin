...
The audit reading is the one that needs stating: in an audit (`review --repo`)
nothing here was just proposed, so the five questions below are asked of what
exists, and an unanswerable question is a finding under the `complexity` lens
rather than a blocker in a plan. Structure that predates the audit and works is
not a defect because it would not be chosen today; it becomes a finding when
the cost is real and someone is paying it.

The rule it all reduces to: **use the simplest structure that still preserves the
business rules, security, testability, and the evolution the brief actually
asks for.** Not the simplest imaginable — the simplest that holds.
...
2. Does that problem exist **today**, in this brief — not in a plausible
...
If the plan shows these, cut before building it:
...
Beyond the per-task fields the task template (`../templates/task.md`)
requires, the plan names the risks that decide whether the structure is
sufficient: volume, concurrency, authorization, partial failure, backward
compatibility, and observability. A risk named here becomes a task or an
accepted risk in the brief. One that is not named is one the reviewer will
raise later, against code.
